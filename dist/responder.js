 ...
 */
async function handleComment(context, pr, summary, claude) {
 ...
 */
function shouldRespond(comment, botUsername) {
    ...
    const hasKeywords = ['what', 'why', 'how', 'explain', 'suggest', 'test'].some((k) => lower.includes(k));
    return isQuestion || isHelpRequest || hasKeywords;
}
/**
 * Process a comment and post response if appropriate
 */
async function processComment(context, pr, summary, claude, github, botUsername) {
