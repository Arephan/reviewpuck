 ...
 */
export function getSizeLabel(lines: number): string {
  if (lines <= 100) return `${SIZE_PREFIX}xs`;
  if (lines <= 250) return `${SIZE_PREFIX}s`;
  if (lines <= 500) return `${SIZE_PREFIX}m`;
  if (lines <= 800) return `${SIZE_PREFIX}l`;
 ...
 */
export async function addSplitLabels(
  github: GitHubClient,
  prNumber: number,
 ...
 */
export function formatSplitClosedComment(
  originalPrNumber: number,
  splits: Array<{ number: number; title: string; order: number }>
): string {
